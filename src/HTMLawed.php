...
namespace ArtisanPackUI\Security\HTMLawed;
 ...
 */
function htmLawed(string $t, mixed $C = 1, mixed $S = []): string
{
    ...
    if ($C['cdata'] || $C['comment']) {
        $t = preg_replace_callback('`<!(?:(?:--.*?--)|(?:\[CDATA\[.*?\]\]))>`sm', 'ArtisanPackUI\Security\HTMLawed\hl_commentCdata', $t);
    }
    $t =
        preg_replace_callback(
            '`&amp;([a-zA-Z][a-zA-Z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`',
            'ArtisanPackUI\Security\HTMLawed\hl_entity',
            str_replace('&', '&amp;', $t));
 ...
 */
function hl_balance(string $t, int $act = 1, string $parentEle = 'div'): string
{
 ...
 */
function hl_tag(array $t): string
{
    $t = $t[0];
    global $C;
            ...
            if ('style' == $attr && ! $C['style_pass']) {
                ...
                if ('srcset' == $attr || ('archive' == $attr && 'applet' == $ele)) {
                    ...
                    foreach (explode(',', $v) as $k => $x) {
                        ...
                        $x = trim($x[0]);
                        if (isset($x[0])) {
                            $vNew .= HTMLawed.phphl_url($x, $attr).(empty($k) ? '' : ' '.$k).', ';
                        }
                    }
                    $v = trim($vNew, ', ');
                }
                if ('itemtype' == $attr || ('archive' == $attr && 'object' == $ele)) {
                    ...
                    foreach (explode(' ', $v) as $x) {
                        if (isset($x[0])) {
                            $vNew .= hl_url($x, $attr).' HTMLawed.php';
                        }
                    }
                    $v = trim($vNew, ' ');
                } else {
